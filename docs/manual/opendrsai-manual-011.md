...
#  OpenDrSai-智能体的函数工具管理组件

OpenDrSai和Autogen支持FunctionTool类型，可以将本地函数工具接入到OpenDrSai中，由函数工具的管理组件-`Workbench`进行统一调用和管理。

## 1.本地/FastAPI/HepAI函数工具的接入
...
## 3.`Workbench`组件管理和执行函数工具

```python
workbench = StaticWorkbench(FunctionTools)

ToolInfo = await workbench.list_tools()
...
DrSaiAgent(
    ...
    tools=FunctionTools # 或者直接传入列表tools，或者传入workbench
    )
```