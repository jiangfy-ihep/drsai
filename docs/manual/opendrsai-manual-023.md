...
# OpenDrSai智能体开发实践-长时间运行任务管理智能体


本文档详细解析了`LongMCPAgent`智能体如何与MCP（Model Context Protocol）远程工具协同工作，实现长时间运行任务的智能执行与管理查询。该系统基于OpenDrSai框架，结合了[OpenDrSai](https://github.com/hepai-lab/drsai/tree/main)案例中`examples/agent_groupchat/groupchat_task_LongTask/groupchat_task/Long_MCP_Agent.py`和`examples/tools/MCP_Long_task/long_task_MCP.py`两个核心组件。
